...
def check_useable(line_id):
    ...
    cur = conn.cursor()
    created_on = ''
    #cur.execute("select TO_CHAR(created_on, 'YYYY/MM/DD HH24:MM:SS') from aism_pay where rtnmsg='Succeeded' and line_id=%s order by created_on desc LIMIT 1",(line_id))
    cur.execute("select TO_CHAR(created_on, 'YYYY/MM/DD HH24:MM:SS') from aism_pay where rtnmsg='Succeeded' and line_id='"+line_id+"' order by created_on desc LIMIT 1")
    for r in cur :
        created_on=r[0]         
    current_time = datetime.now() + timedelta(hours=8)
